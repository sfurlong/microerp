<html>
<head>
<title>Cash Disbursements By Account Results</title>
</head>
...
<% //VARIABLE DECLARATIONS
   ...
   String acctId = request.getParameter("parm2");
   String showDates="";
   String sqlStatement="";
   String whereClause="";
...
<% if ((! startDate.trim().equals("")) && (! endDate.trim().equals("")))
   {
     ...
     whereClause = "(date_paid between '" + startDate + "' and '" + endDate + "') and ";
   }
   else
     whereClause = "";

   //Create and execute SQL statement
   sqlStatement="select total_shipping_charges, acctid, acctname, check_num, id, date_paid, vendor_name, invoice_num, total_payments_posted from payment_voucher where ";
   whereClause+="(is_voided is null or is_voided = 'N') and acctid like '" + acctId + "' order by acctid, check_num";
...
<tr bgcolor=darkblue>
	...
	<td><FONT color=white><B><center><U>Check Num</center></U></B></td>
	<td><FONT color=white><B><center><U>Voucher Id</center></U></B></td>
	<td><FONT color=white><B><center><U>Date Paid</center></U></B></td>
        <td><FONT color=white><B><center><U>Vendor Name</U></B></td>
	<td><FONT color=white><B><center><U>Invoice Num</center></U></B></td>
	<td><FONT color=white><B><center><U>Payment Amt</center></U></B></td>
</tr>
...
<% //process results for display
   for (int i=0; i < results.size(); i++)
   {
     ...
     else
     {
       ...
       if (i > 0) //not first record
         display+="<td></td><td></td><td></td><td></td><td></td><td></td><td align=right><FONT size=2>Total:</font></td>"
     ...
     //display check num, voucher id, date paid and vendor name
     ...
     display+="<td align=left><FONT size=2>" + colVect.get(6).toString() + "</font></td>";

     //display invoice number, replace nulls with "-"
     String temp2 = "" + colVect.get(7);
     ...
     //display payment amt and add it to running grand/group totals (subtract shipping charges from each)
     display+="<td align=right><FONT size=2>" + daiFormatUtil.doubleToCurrency((Double.parseDouble(colVect.get(8).toString()) - Double.parseDouble(temp)), true) + "</font></td>";
     grandTotal += Double.parseDouble(colVect.get(8).toString()) - Double.parseDouble(temp);
     groupTotal += Double.parseDouble(colVect.get(8).toString()) - Double.parseDouble(temp);

   }//end for i

   //display the last group total
   display+="</tr><td></td><td></td><td></td><td></td><td></td><td></td><td align=right><FONT size=2>Total:</font></td>"
           + "<td align=right><FONT size=2><u>" + daiFormatUtil.doubleToCurrency(groupTotal, true) + "</u></font></td></tr><tr>";
   ...
   + "<td align=center><FONT size=2>-</font></td><td align=center><FONT size=2>-</font></td><td align=right><FONT size=2>Total Shipping Charges:</font></td>"
   + "<td align=right><FONT size=2>" + daiFormatUtil.doubleToCurrency(shippingTotal, true) + "</font></td></tr>";
   display+="<tr><td></td><td></td><td></td><td><td></td></td><td></td><td align=right><FONT size=2>Grand Total:</font></td>"
