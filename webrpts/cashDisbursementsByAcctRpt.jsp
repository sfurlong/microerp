...
<html>
<head>
<title>Cash Disbursements By Account</title>
...
<TABLE BORDER=0 CELLSPACING=0 CELLPADDING=0 bgcolor="#33cccc" width=100%>
  <tr bgcolor="#33cccc">
	<td colspan=2 align=center><H1>Cash Disbursements By Account</H1></td>
  </tr>
  ...
  <tr>
    ...
    <select name=parm2 size=5>
        <option value="%" selected> --All Accounts--
	    <% 
        for (int i=0; i<acctRecSet.getSize(); i++) { 
            String acctName = acctRecSet.getRec(i).getAttribVal(accountObj.DESCRIPTION);
            String acctId   = acctRecSet.getRec(i).getAttribVal(accountObj.ID);
        %>
             <option value="<%=acctId%>"> <%=acctId + "-" + acctName%>
	    <% } %>
	</select>
    </td>
  </tr>
  ...
  <tr>
	<td align="right"><input type=hidden  value="cashDisbursementsByAcct.xml" name=rptName  size=15>
