...
<TITLE>Cash Receipts (Checks)</TITLE>
...
String sqlExp = " account_type = 'BANK' order by id ";
Vector vect = dbAdapter.queryByExpression(security, new accountObj(), sqlExp);
String[] accts = new String[vect.size()];

for (int i=0; i<accts.length; i++) {
    accountObj obj = (accountObj)vect.elementAt(i);
    accts[i] = obj.get_id() + " " + obj.get_description(); 
} 
...
<tr bgcolor="#CCFFCC">
	<td colspan=2 align=center><H1>Cash Receipts (Checks)</H1></td>
</tr>
...
<tr>
    ...
    <% //for each option, value is first 8 chars of returned string
       for(int i=0; i<accts.length; i++) { %>
         <OPTION VALUE= <%= accts[i].substring(0,8) %> > <%= accts[i] %> <BR> <%  } %>
    </SELECT></td>
</tr>
...
<tr>
	<td align="right"><input type=hidden  value="cashReceipts.xml" name=rptName  size=15>
