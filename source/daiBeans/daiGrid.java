...
import com.altaprise.plaf.ThemeUtilities;
...
public class daiGrid extends JPanel {
        ...
        _theGrid.getSelectionModel().addListSelectionListener(new javax.swing.event.ListSelectionListener()
                {
                    ...
                    {
                        if (_gridController != null && !_gridController.isDetailCommitted()) {
                            if (_activeRow != getSelectedRow()) {
                                JOptionPane.showConfirmDialog(null,"The current record has unsaved changes.  " +
	...
	public void createColumns(int numCols) {
		...
		for (int i=0; i<numCols; i++)rowTemplate[i] = new String();
     ...
     * @param action
     */
    public void addKeyMapAction(javax.swing.KeyStroke keyStroke, javax.swing.Action action) {
	...
	private void setTableCellRenderers(int numCols) {
		for (int i=0; i<numCols; i++) {
		    TableCellEditor tce = _theGrid.getDefaultEditor(_gridModel.getColumnClass(i));
			...
			TableCellRenderer tcr = new daiTableCellRenderer(_theGrid.getDefaultRenderer(_gridModel.getColumnClass(i)));
	...
	public void addRow() {
		...
		if (_showRowNumbers) showRowNumbers();
	}
	...
	public void stopCellEditing(int row, int col) {
		try {
			...
			System.out.println("daiGrid.stopCellEditing Error: " + e.getLocalizedMessage());
		}
	}
	...
	class daiTableCellRenderer extends DefaultTableCellRenderer {
		...
		public Component getTableCellRendererComponent(JTable table, Object value,
			...
			Component c = _tcr.getTableCellRendererComponent(table, value, isSelected, hasFocus, row, column);
	...
	public static void main(String[] args) {
		...
		grid.setHeaderNames(new String[] { "h", "OBJ", "Item Id", "Description",
